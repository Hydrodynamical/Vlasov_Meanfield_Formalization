...
discipline that keeps the development sound at speed. The conceptual framing is
the contribution; the case study is its evidence, and we state that evidence's
quantitative claims as observations of one game --- with one director, one system,
at one moment in a fast-moving field --- rather than as general laws.
\end{abstract}
...
layer). And there are three phases,
...
condition, and its reusability grade. Section~\ref{sec:phases} describes the three phases and
their dominant strategies. Section~\ref{sec:strategies} presents the
...
the reusability actually achieved. Appendix~\ref{app:arc} states the full
mathematical arc, including the superposition principle and the regularity it
requires, each statement paired with its Lean form.
...
Erd\H{o}s-problem papers. These differ from the present work on two axes: the
agent \emph{drives} (the human, when present, supervises), and the targets are
\emph{many short statements} --- competition or Erd\H{o}s problems. Our regime is
the complementary one --- human-directed, \emph{one} substantial result carried
end to end --- where the scarce resource is the mathematician's judgment about how
to decompose a hard theorem and how to triage the ambient library's gaps, not raw
proof search over short goals. Tellingly, the autonomous systems exhibit the very
...
apparatus we foreground. The ``Agentic Researcher'' guide of Zimmer et al.\
