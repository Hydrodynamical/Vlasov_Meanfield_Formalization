...
dependency-isolated behind a four-name interface to the kinetic-theory content,
with no reverse dependency --- a candidate self-contained layer
(Definition~\ref{def:selfcontained}), roughly a fifth of the development's
declarations.
% TODO(verify-at-HEAD): once ot-standalone re-sync + lake build + runLinter pass
% at HEAD, upgrade ``candidate self-contained layer'' to ``self-contained layer''
% (standalone-compiled against Mathlib, linter-clean).
...
content reaches it through an interface of exactly four names, with no edge in the
...
the concision editing that an upstream submission would still require.
...
was won, we reorganized the optimal-transport machinery into a candidate
self-contained layer (Definition~\ref{def:selfcontained}) and read the
declaration-level dependency graph off the proof assistant's elaboration
environment. The cut is one-directional: the kinetic-theory content reaches the
optimal-transport layer through an interface of width $w = 4$ (four names), and
the number of edges running the other way --- any optimal-transport declaration
depending on a kinetic-theory one --- is exactly $0$ (check Q1). Directionality
and interface width, not the size of the layer, are what make it upstreamable;
the $0$ is the hard fact a referee can re-derive from the graph.
% TODO(reuse-rerun): regenerate w, the reverse-edge count, and the declaration
% counts below by re-running formalize/phase-d/reuse-score.lean with the
% WeakToLagrangian bridge module in scope (the last run predated the bridge); do
% not hand-edit these numbers.

(These ``four names'' count the interface \emph{declarations} the kinetic content
imports from the optimal-transport layer; the number is unrelated to the four
\emph{conjuncts} of the \lean{IsLagrangianVlasovSolution} predicate displayed in
Appendix~\ref{app:arc}, which a reader meeting both might otherwise conflate.)

\paragraph{Toward a compiled certificate.}
Check (Q1) is an audit of a graph we extract; checks (Q2) and (Q3) are designed to
turn that audit into artifacts a referee re-runs, and we have not yet run them at
the current revision. The intended (Q2) certificate packages the layer as a Lake
project declaring the ambient library as its only dependency, with zero
kinetic-theory files in its import graph, so that a successful build \emph{would}
itself prove isolation --- the build cannot succeed if a hidden reverse edge
exists; (Q3) is the ambient library's own linter suite, the automated gate its
maintainers apply before any merge. We state these as the checks the grade
\emph{requires} rather than as ones already compiled; until they pass at the
current revision, the extracted machinery is a \emph{candidate} self-contained
layer.
% TODO(verify-at-HEAD): the standalone package was last built against an earlier
% snapshot of the layer and has since drifted (~240 lines); the linter has not yet
% been run at all. Re-sync ot-standalone to HEAD, ``lake build'' it against current
% Mathlib, and run runLinter, THEN state the (Q2)/(Q3) results here as achieved
% facts. Until then, do NOT assert standalone compilation or linter-cleanliness.

\paragraph{Size, reported descriptively.}
The layer's size is a descriptive statistic, not the grade. By declaration count
it is $56$ of the development's $272$ top-level declarations --- roughly one in
five. Other denominators move the figure: a layer can be a small fraction of the
declarations yet a large fraction of the hard-won proof effort, or the reverse.
That is exactly why size is reported as an aside rather than as the certificate ---
the grade is the pass on (Q1)--(Q4), and the fraction only indicates whether a
layer is large enough to be worth extracting at all.

\paragraph{What is asserted rather than compiled.}
The one judgment we do not compile is (A1): the layer's declarations are in the
ambient library's conventional shape and free of project-specific jargon, but we
do not claim they meet its concision and idiom standards as written. Bringing them
to that bar is editing work we have not done, so the layer is a \emph{candidate}
upstream contribution --- a finite Kantorovich duality, a transport-polytope
compactness theory, a coupling-gluing construction, and the $\Wone/\Wbar$ and
coupling API, dependency-isolated and independent of the kinetic-theory
application that motivated it --- not a finished one. The extraction itself was
performed move by move, with the axiom footprint re-verified after each move to
confirm the reorganization changed only \emph{where} declarations live, never
\emph{what} the target theorems depend on.
...
this case the extracted layer is dependency-isolated but not yet brought to the
ambient library's concision standard, so it remains a candidate for upstreaming
...
extracting a candidate self-contained optimal-transport layer
(Definition~\ref{def:selfcontained}). The certification is
