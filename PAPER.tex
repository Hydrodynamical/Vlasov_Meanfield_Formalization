...
separated into a self-contained layer (Definition~\ref{def:selfcontained}) that
builds against Mathlib alone and passes its linter suite --- roughly a fifth of
the development's declarations, reached through a $17$-declaration interface, with
...
content reaches it through a $17$-declaration interface, with no edge in the
...
layer (Definition~\ref{def:selfcontained}). The cut, read off the
declaration-level dependency graph in the proof assistant's elaboration
environment, is one-directional: the kinetic-theory content reaches the
optimal-transport layer through a $17$-declaration interface --- the
Wasserstein-1 metric API, the coupling predicate \lean{IsCoupling}, and the two
phase-space types --- and \emph{zero} edges run the other way. Directionality and
interface width, not the size of the layer, are what make it upstreamable; the
$0$ is the hard fact a referee re-derives from the graph.
...
import graph --- the layer builds against Mathlib alone (Q2), and passes Mathlib's
environment-linter suite ($16$ linters) with zero warnings (Q3). That Q3 pass is
...
(Q2) standalone build against Mathlib alone & exit $0$ \\
(Q3) environment linters / warnings & $16$ / $0$ \\
axiom footprint of the target theorems & \cleanfootprint{} \\
size: general / total declarations & $60/299 \approx 20\%$ \\
interface width $w$ & $17$ \\
\hline
\end{tabular}
\end{center}
...
extracting a self-contained optimal-transport layer that compiles against Mathlib
alone and is linter-clean (Definition~\ref{def:selfcontained}). The certification is
